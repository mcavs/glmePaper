...
Orthodont %>%
  ggplot(aes(x = age, y = distance, color = Sex)) + geom_point() +
...
library(glme) # for applying glme()
library(BHH2) # for retrieving penicillin data
penicillin.data %>%
  ggplot(aes(x = run, y = yield, color = blend)) + geom_point() +
  geom_smooth(method = "lm", se = FALSE)
...
Blackmore %>%
  ggplot(aes(x = age, y = exercise, color = group)) + geom_point() +
  geom_smooth(method = "lm", se = FALSE)
...
sleepystudy %>%
  ggplot(aes(x = Days, y = Reaction, color = Subject)) + geom_point() +
  geom_smooth(method = "lm", se = FALSE)
